...
class InstagramDownloader(BaseDownloader):
    ...
    def get_metadata(self, url: str) -> dict:
        ...
        try:
            ...
            if 'Private' in error_msg or 'login' in error_msg.lower():
                raise DownloadError(
                    f"Instagram post is private or requires login. "
                    f"Please use a public post URL. URL: {url}"
                )
            elif 'not found' in error_msg.lower() or '404' in error_msg:
                raise DownloadError(
                    f"Instagram post not found. Please check the URL. URL: {url}"
                )
            else:
                raise DownloadError(
                    f"Failed to get video metadata: {error_msg}. URL: {url}"
                )
    ...
    def download(self, url: str) -> Video:
        ...
        try:
            ...
            raise DownloadError(
                f"Failed to download Instagram video: {str(e)}. URL: {url}. "
                f"Suggestions: 1) Check internet connection, 2) Verify post is public, "
